...
const props = defineProps({
  ...
  role: { type: String, default: 'PhotoEditor' },
});

// Search and filters
const searchQuery = ref('');
const statusFilter = ref('all'); // all | active | blocked
const filteredUsers = computed(() => {
  const q = searchQuery.value.trim().toLowerCase();
  const base = props.users.filter((u) => {
    if (statusFilter.value === 'active' && !!u.is_blocked) return false;
    if (statusFilter.value === 'blocked' && !u.is_blocked) return false;
    return true;
  });
  if (!q) return base;
...
function submitCreate() {
  createForm.post(route('users.photo_editors.store'), {
    onSuccess: () => {
      createForm.reset();
...
<template>
  ...
  <Head title="Пользователи — Фоторедакторы" />
  <TablerLayout>
    <template #header>Пользователи / Фоторедакторы</template>
          ...
          <div class="card-table">
            <div class="card-header">
              <div class="row w-full">
                <div class="col">
                  <h3 class="card-title mb-0">Фоторедакторы</h3>
                  <p class="text-secondary m-0">Управление пользователями с ролью Фоторедактор.</p>
                </div>
                  ...
                  <div class="ms-auto d-flex flex-wrap btn-list">
                    <div class="input-group input-group-flat w-auto me-2">
                      ...
                      <input id="advanced-table-search" v-model="searchQuery" type="text" class="form-control"
                        autocomplete="off" placeholder="Поиск (имя, email, ФИО)" />
                    </div>
                    <div class="w-auto me-2">
                      <select class="form-select" v-model="statusFilter" aria-label="Фильтр статуса">
                        <option value="all">Все</option>
                        <option value="active">Только с доступом</option>
                ...
                <table class="table table-vcenter table-selectable">
                  ...
                  <tbody class="table-tbody">
                    <tr v-if="displayedUsers.length === 0">
                      <td colspan="9" class="text-center text-secondary py-4">Нет пользователей для отображения</td>
                    </tr>
                    <tr v-for="(u, idx) in displayedUsers" :key="u.id">
                      <td>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Добавить фоторедактора</h5>
