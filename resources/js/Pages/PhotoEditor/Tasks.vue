...
function matchesFilter(s) {
  ...
  if (statusFilter.value === 'assigned') return isAssigned(s) && !['on_review','accepted','rejected'].includes(s.status);
...
// Offcanvas state and logic (no assignee controls, no accept/reject here)
...
function openOffcanvas(s) {
  const brandName = s.task?.brand?.name || '';
  oc.value = {
    brandId: s.task?.brand_id || s.task?.brand?.id || null,
    brandName,
    taskId: s.task?.id || null,
    taskName: s.task?.name || '',
  };
  subtaskId.value = s.id;
  currentSubtask.value = { ...s };
  activeOcTab.value = 'comments';
  comments.value = [];
  newComment.value = '';
  loadComments();
  // Load PhotoEditor files and Photographer public URL for Information tab
  loadYandexFiles();
...
async function loadComments() {
  if (!subtaskId.value || !oc.value.brandId || !oc.value.taskId) { comments.value = []; return; }
  try {
...
// Photographer public URL shown in Information tab
...
function yandexFolderPath() {
  if (!oc.value.brandName || !currentSubtask.value) return null;
  const subName = currentSubtask.value.name || '';
  if (!subName) return null;
  // PhotoEditor folder uses prefix "д_" without intermediate Task folder
  return `disk:/${oc.value.brandName}/д_${subName}`;
}
...
function photographerFolderPath() {
  ...
  const subName = currentSubtask.value.name || '';
  if (!subName) return null;
  // Photographer folder uses prefix "ф_" without intermediate Task folder
...
const canSendForReview = computed(() => currentSubtask.value && currentSubtask.value.status !== 'accepted' && currentSubtask.value.status !== 'on_review');
const sendLabel = computed(() => currentSubtask.value?.status === 'rejected' ? 'Отправить на повторную проверку' : 'Отправить на проверку');

async function sendForReview() {
  if (!subtaskId.value || !oc.value.brandId || !oc.value.taskId) return;
  sending.value = true;
  try {
    router.put(
      ...
      { status: 'on_review' },
      {
        preserveScroll: true,
        onSuccess: () => {
          if (currentSubtask.value) currentSubtask.value.status = 'on_review';
          // Also reflect in list
          const idx = props.subtasks.findIndex(x => x.id === subtaskId.value);
          if (idx >= 0) props.subtasks[idx].status = 'on_review';
        },
      }
    );
...
<template>
  <TablerLayout>
    <Head title="Мои задания (Фоторедактор)" />
    <template #header>Мои задания — Фоторедактор</template>
      ...
      <div class="card-header">
        <div class="row w-full">
          <div class="col">
            ...
            <p class="text-secondary m-0">Список подзадач роли «Фоторедактор», назначенных вам.</p>
          </div>
        ...
        <table class="table table-vcenter card-table">
          ...
          <tbody>
            <tr v-if="displayed.length === 0">
              <td colspan="5" class="text-center text-secondary py-4">Назначенных подзадач нет</td>
            </tr>
            <tr v-for="s in displayed" :key="s.id">
              <td>{{ s.task?.name || '—' }}</td>
              <td>{{ s.task?.brand?.name || '—' }}</td>
              <td>
                <span class="badge text-light" :class="statusClass(s.status, !!s.assignee_id)">{{ statusLabel(s.status, !!s.assignee_id) }}</span>
              </td>
              <td>{{ new Date(s.created_at).toLocaleString('ru-RU') }}</td>
              <td class="text-nowrap">
                <button class="btn btn-sm btn-opacity-purple" @click="openOffcanvas(s)">Детали</button>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="offcanvas-body">
          <div class="mb-3">
            <ul class="nav nav-pills">
              <li class="nav-item">
                <button type="button" class="nav-link" :class="{ active: activeOcTab === 'comments' }" @click="activeOcTab = 'comments'">Информация</button>
              </li>
              <li class="nav-item">
                <button type="button" class="nav-link" :class="{ active: activeOcTab === 'files' }" @click="activeOcTab = 'files'">Файлы</button>
              </li>
            </ul>
          </div>
          ...
          <div v-if="activeOcTab === 'files'">
              ...
              <div v-else>
                ...
                <ul v-else class="list-group">
                  <li v-for="it in yandexItems" :key="it.resource_id || it.path || it.name" class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <div>
                      <button v-if="it.type === 'file'" class="btn btn-sm btn-outline-danger" :disabled="currentSubtask?.status === 'accepted'" @click="() => deleteYandexItem(it)">Удалить</button>
                    </div>
                  </li>
                </ul>
              </div>
            ...
            <div class="mt-3 d-flex align-items-center gap-2">
              ...
              <button class="btn btn-primary" :disabled="uploading || currentSubtask?.status === 'accepted'" @click="openUploader">
          ...
          <div v-else>
            <div class="mb-3" v-if="currentSubtask">
              <div class="d-flex align-items-center gap-2 flex-wrap">
                ...
                <span class="badge text-light" :class="statusClass(currentSubtask.status, !!currentSubtask.assignee_id)">{{ statusLabel(currentSubtask.status, !!currentSubtask.assignee_id) }}</span>
                <button class="btn btn-warning btn-sm" :disabled="!canSendForReview || sending" @click="sendForReview">{{ sendLabel }}</button>
              </div>
            </div>

            <!-- Photographer public folder URL -->
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                <button class="btn btn-outline-primary" type="button" :disabled="!photographerPublicUrl" @click="() => window.open(photographerPublicUrl, '_blank')">Перейти</button>
                <button class="btn btn-outline-secondary" type="button" :disabled="!photographerPublicUrl" @click="() => copyText(photographerPublicUrl)">Скопировать</button>
              </div>
            </div>
