...
const checkScroll = () => {
    if (!hasMore.value || loading.value || isRestoringScroll.value) return;
...
async function submitBulkAssign() {
    ...
    await router.put(route('tasks.bulk_update'), { ids, assignee_id: uid }, {
        ...
        onSuccess: () => {
            closeBulkAssign();
            fetchPage(true); // Fetch updated data from backend
            clearSelection();
...
function onArticleSearchInput() {
    // User is typing manually: clear previously selected article to enforce a fresh selection
    if (createForm.value.article_id) {
...
// Load articles for CREATE modal based on brand
...
async function submitCreate() {
    // Check if the selected article exists in the list of available articles
    const selectedArticle = brandArticles.value.find(a => a.id == createForm.value.article_id);
    if (!selectedArticle) {
        alert('Пожалуйста, выберите артикул из списка. Ручной ввод не допускается.');
        return;
    }
...
const editForm = ref({ name: '', brand_id: '', task_type_id: '', article_id: '', assignee_id: '', priority: 'medium', source_files: [''] });
...
function closeEdit() {
    ...
    editForm.value = { name: '', brand_id: '', task_type_id: '', article_id: '', assignee_id: '', priority: 'medium', source_files: [''] };
    editArticleSearch.value = '';
}
...
watch(() => editForm.value.brand_id, async (newBrandId) => {
    ...
    editArticleSearch.value = '';
    // Load articles for the newly selected brand
    if (newBrandId) {
...
// Comment image upload (supports multiple images)
...
async function addComment() {
    ...
    try {
        ...
        // If multiple images selected, send one request per image.
        const files = Array.isArray(selectedCommentImages?.value) ? selectedCommentImages.value : [];
        if (files.length > 0) {
            for (let i = 0; i < files.length; i++) {
                const formData = new FormData();
                // Send content only with the first image
                ...
                if (!res.ok) {
                    const errorData = await res.json().catch(() => ({}));
                    if (errorData?.errors?.content?.[0]) alert(errorData.errors.content[0]);
...
function initSourceFilesFromTask(task) {
    ...
    const cleaned = list.map(v => (v ?? '').toString());
...
async function saveSourceFiles() {
    ...
    try {
        await router.put(route('brands.tasks.update', { brand: sourceOc.value.brandId, task: sourceOc.value.taskId }), payload, {
            preserveScroll: true,
            onSuccess: () => {
                // Update local list row optimistically
                const idx = items.value.findIndex(x => x.id === sourceOc.value.taskId);
                if (idx !== -1) {
                    items.value.splice(idx, 1, { ...items.value[idx], source_files: files });
                }
                toast.success('Файлы исходника сохранены');
            }
        });
    } catch (e) {
        console.error(e);
        alert('Не удалось сохранить файлы исходника');
    }
}
...
async function deleteSourceComment(c) {
    if (!sourceOc.value.taskId) return;
    // Enforce UI rule also in logic: only last manager/admin own comment can be deleted
    if (!canDeleteSourceComment(c)) {
        try { toast.error('Удаление запрещено'); } catch { }
        return;
    }
...
function canDeleteSourceComment(c) {
    ...
    if (!user || !Array.isArray(sourceComments.value) || sourceComments.value.length === 0) return false;
    const last = sourceComments.value[sourceComments.value.length - 1];
    const authorId = c?.user_id ?? c?.user?.id;
    return Boolean(isManagerOrAdmin.value && last?.id === c?.id && authorId && authorId === user.id);
}

// Copy first source file link to clipboard with robust fallbacks
const copySourcePublicLink = async (task) => {
    const link = String(task?.source_files?.[0] ?? '').trim();
    if (!link) return;
...
<template>
    <ContentLayout>
                            ...
                            <tr v-for="t in displayedTasks" :key="t.id" style="font-size: 13px !important">
                                ...
                                <td class="text-center" style="vertical-align: middle;">
                                    <div class="d-flex gap-1">
                                        <button class="btn btn-icon btn-ghost-secondary"
                                            @click="openSourceCommentsOffcanvas(t)"><svg
                                        ...
                                        <button class="btn btn-icon btn-ghost-primary"
                                            @click="openSourceFilesOffcanvas(t)"><svg xmlns="http://www.w3.org/2000/svg"
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <button class="btn btn-icon btn-ghost-primary"
                                            @click="openFilesOffcanvas(t)"><svg xmlns="http://www.w3.org/2000/svg"
                                                width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                stroke-linejoin="round"
                                                class="icon icon-tabler icons-tabler-outline icon-tabler-files">
        ...
        <!-- Source Offcanvas: BRAND / Исходник with Comments and Files tabs -->
        <teleport to="body">
                ...
                <div class="offcanvas-body">
                        ...
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <button type="button" class="nav-link"
                                    :class="{ active: sourceActiveTab === 'comments' }"
                            ...
                            <li class="nav-item">
                                <button type="button" class="nav-link" :class="{ active: sourceActiveTab === 'files' }"
                                    @click="() => { sourceActiveTab = 'files'; const t = items.find(x => x.id === sourceOc.taskId); if (t) initSourceFilesFromTask(t); }">
                                    Файлы
                                </button>
                            </li>
                        </ul>
                        ...
                        <div v-else>
                            ...
                            <ul class="list-unstyled">
                                <li v-for="c in sourceComments" :key="c.id"
                                    ...
                                    <button v-if="canDeleteSourceComment(c)" class="btn btn-ghost-danger btn-sm ms-2"
                                        title="Удалить" @click="deleteSourceComment(c)">Удалить</button>
                                </li>
                            </ul>
                            <div class="mt-3">
                    ...
                    <div v-else>
                        ...
                        <div v-for="(f, idx) in sourceFiles" :key="idx" class="input-group mb-2">
                            ...
                            <button type="button" class="btn btn-outline-danger" @click="removeSourceFilesField(idx)"
                                :disabled="sourceFiles.length <= 1">-</button>
                        </div>
